...
- 接收总指挥写入的 `cursor_decision.json`
- 触发执行链路
- 生成 `cursor_feedback.txt`
- 持久化运行状态与任务结果文件
...
1. 主 Agent 通过 MCP `write_decision` 写入 `cursor_decision.json`
2. `run_skill.py` 读取决策并触发执行
3. `dynamic_agent_skill.py` 承接执行与反馈汇总
4. 结果写入 `cursor_feedback.txt`
5. 插件监听反馈文件并复制到剪贴板
6. 用户将反馈粘贴回 Chat
...
1. **优先使用 Cursor 内置模型**
   ...
   - 默认模式为 Cursor `Auto`
   - 目标是优先走 Cursor 内置模型链路，而不是优先走 `.env` 中 API 模型链路

2. **若 Cursor 内置模型链路失败，则自动 fallback 到 API 链路**
   - 当前运行时 backend 已支持该降级顺序
   - 但目前仓库内**尚未接入稳定的 Python -> Cursor 内置模型桥接器**
   - 因此当前实现会先尝试 `cursor_builtin`，随后因“未接入桥接器”自动降级到 API 模型候选链路
