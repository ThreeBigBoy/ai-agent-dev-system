...
function activate(context) {
    ...
    if (!workspaceFolder) {
        vscode.window.showErrorMessage("❌ 请先打开 agent_team_project 项目目录，再激活插件！");
        return;
    }
    const feedbackFile = path.join(workspaceFolder, 'cursor_feedback.txt');
    if (!fs.existsSync(feedbackFile)) {
        try {
            fs.writeFileSync(feedbackFile, '', 'utf-8');
        } catch (e) {
            vscode.window.showWarningMessage("⚠️ 无法创建 cursor_feedback.txt，请先运行一次 Skill 生成该文件后再依赖插件监听。");
        }
    }
        ...
        if (eventType === 'change') {
            setTimeout(() => {
                try {
                    ...
                    const messageForChat = `### Agent团队执行反馈\n${feedbackContent}\n请根据反馈判断是否需要调整任务分工，输出新的JSON决策并更新cursor_decision.json`;
    ...
    let disposable = vscode.commands.registerCommand('cursor-agent-extension.listenFeedback', () => {
        try {
            if (fs.existsSync(feedbackFile)) {
                ...
                vscode.window.showInformationMessage("⚠️ 尚无反馈文件，请先执行 Skill 生成 cursor_feedback.txt");
            }
        } catch (e) {
    ...
    vscode.window.showInformationMessage("✅ Cursor Agent 反馈插件已激活，监听 cursor_feedback.txt，反馈将复制到剪贴板并提示粘贴到 Chat。");
}
