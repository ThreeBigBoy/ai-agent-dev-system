...
**补充方法论**：解析完成后或当无图片需产出线框图、流程图、状态图、时序图等时，可使用 **Mermaid** 在 Markdown 中生成图表，存放于 `design/documents/[change-id]/design-assets/` 并在 PRD 中引用。书写规范、图表类型选择与自检见 **REFERENCE**《Mermaid 使用规范与技巧》：`ai-agent-dev-system/skills/image-analysis/REFERENCE/mermaid-usage-norms-and-tips.md`。
...
- **前置资料来源（解析前建议读取）**：
  ...
  - `design/documents/[change-id]/` 下的功能需求说明书等（若已存在）；
...
4. **纳入需求与 spec**
   - 将解析结果写入或补充到：
     - `design/documents/[change-id]/功能需求说明书.md`（或等价文档）中的界面/交互描述；
   ...
   - **可选**：根据解析结果或需求，用 **Mermaid** 产出流程图、状态图、时序图、页面跳转图等，写入 `design/documents/[change-id]/design-assets/flows/` 或 `wireframes/`，并遵循 REFERENCE《Mermaid 使用规范与技巧》；在 PRD 或需求说明中引用对应 .md。
...
- **与 request-analysis**：需求分析时若涉及前端或含设计图/截图，request-analysis 调用本技能解析图片，并将结果纳入 `design/documents/[change-id]/` 与 `openspec/changes/[change-id]/specs/*/spec.md` 的场景描述。
...
3. image-analysis：
   ...
   - 将结构化描述写入 `design/documents/[change-id]/功能需求说明书.md` 的「商品详情页」小节，并同步到 `openspec/changes/[change-id]/specs/.../spec.md` 中相关 Requirement 的 Scenario（如「当用户进入详情页时，页面呈现…」）。
