...
| **新建 OpenSpec 变更 / 新建 change-id** | 用户发起新需求、新建变更或迭代，第一步执行前 | 必须先读 `OpenSpec.md` 第六节与 4.3 节，以及 `memory/patterns/pattern-openspec-change-workflow.md`，再建 design/documents 与 openspec/changes；**若变更产出含脚本/工具**，须另读 `memory/patterns/pattern-openspec-scripts-and-spec-for-tooling.md`，按其中约定：脚本放在项目根 `scripts/` 下子目录（.py + README），且对归属的 capability 建 `openspec/changes/[change-id]/specs/[capability]/spec.md` 增量 | `projects-rules-for-agent.md` 2.1 |
...
| **变更实施前方案评审** | 用户或主 Agent 要求在按 tasks.md 实施前对变更做方案详细评审（查缺补漏）时 | 必须先读 `memory/patterns/pattern-change-pre-implementation-review.md`，并按其中步骤执行：OpenSpec 6.2 符合性、规则/记忆审视、四维方案评审、文档间一致性、任务统计一致；产出评审记录至 `design/documents/changes/[change-id]/records/`，同步更新方案细节 | 本 pattern；主 Agent 执行评审时加载 |
