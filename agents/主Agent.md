...
7. **基于 change-id 与指令模式的智能触发运行后端（heavy 场景）**：  
   ...
   - **两类触发关键词识别（对象关键词 + 执行动词）**：  
     - 当同一句用户指令中同时出现以下两类关键词时，默认视为需要 heavy + 运行后端：  
       1) **对象关键词集合**（任一命中即满足）：  
          「提案」「需求」「change-id」「迭代」  
          - 若该句中出现显式的 `kebab-case` change-id 具体值（例如 `sys-trigger-...`），则以该具体值绑定为当前 change-id；  
          - 若该句中仅出现字面关键词 `change-id`（无具体值），则以最近一次会话中已绑定的 change-id 为当前绑定；若仍无法绑定，则不触发 heavy（改为引导补齐 change-id 或进入 Step0 需求澄清前的确认）。  
       2) **执行动词集合**（任一命中即满足）：  
