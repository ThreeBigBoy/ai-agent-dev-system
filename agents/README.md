...
### 宿主原生多 Agent / 子 Agent 能力

- **本质**：宿主提供的独立 Agent 实例或 Chat 模式，可单独配置提示、工具、模型，部分宿主支持并行执行。
...
- **主会话以「主 Agent」身份运行**  
  通过宿主入口文件约定先读 `主Agent.md`，由主 Agent 做任务拆解、审核、进度与闭环。

- **运行层可以有多种实现**  
  - 方式 A：同一会话按需读取 `子Agent-xxx.md` 串行执行。
  - 方式 B：通过宿主原生多 Agent / 子 Agent 机制做并行执行。
