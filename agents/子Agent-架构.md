...
# 配额与模型（本角色硬约束）
- **核心场景（Composer + Kimi 组合）**：0-1 工程搭建、核心架构决策、重大技术风险评估、核心 OpenSpec 规范排查等关键任务，优先在当前会话中组合使用 Composer 系列（与 CLI/代码强绑定、便于多文件读写与验证）与 Kimi K2.5 / K2（长上下文技术方案推演、中文文档推理），并按 `projects-rules-for-agent.md` 第 6.1、6.2 条的中国区配额策略执行。  
- **日常场景（主力/慢速）**：技术规范编写、工程结构分析、CLI 执行、普通代码评审、日常问题排查，优先使用 Composer 系列与主力模型；简单代码评审、技术问答、规范校验等用轻量/慢速模型，减少快速额度消耗。  
- **外部模型复核（仅极高复杂度/高风险场景）**：当任务属于大型架构设计或重构评审、整仓级 code review / 安全审计、需要极长上下文的一次性技术难题等，并且在 Composer + Kimi 范围内已给出尽力方案但仍存在重大不确定性时，应配合主 Agent 按 `projects-rules-for-agent.md` 第 6.3 条，在回复中**明确建议用户：可手动切换到接入外部第三方 API（如 DeepSeek 等）的会话，对关键技术方案做二次 review / 推演后再决策**。  
- 若需在本角色下使用按 API 计费的 **other models**（Dashboard “Consumed by other models…” 所指），**必须同时满足** `projects-rules-for-agent.md` 第 6.2 条中对架构/技术侧高复杂度场景的强制约定，仅在上述高复杂度/高风险场景下才可启用；日常工程结构分析、普通 code review、技术问答等**一律不得消耗该额度**。
